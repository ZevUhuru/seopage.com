...
export default function ReplayChooser() {
  return (
    <main className="px-6 py-16 sm:px-10 lg:px-24 lg:py-24">
      <h1 className="nh-display text-[clamp(56px,8vw,120px)] leading-[0.9]">The app section.</h1>
      <p className="mt-6 max-w-[640px] text-[19px] leading-[1.55] text-[#C9D0E2]">
        Three ways to show the builder on a trade page, with real images generated through api.esy.com. The switcher at the bottom flips direction and trade.
      </p>
        ...
        {REPLAY_STYLES.map((s) => (
          <li key={s.key} className="flex flex-col gap-4 rounded-[24px] border border-white/12 bg-[#0A0F1E] p-7">
            ...
            <span className="mt-auto flex flex-wrap gap-3 pt-4">
              {VERTICALS.map((v, i) => (
                <Link key={v.slug} href={`/prototypes/replay/${s.key}/${v.slug}`} className={`flex h-11 items-center rounded-full px-5 text-[14.5px] ${i === 0 ? "bg-[#3D6BFF] font-semibold text-white hover:bg-[#5A82FF]" : "border border-white/30 hover:border-white/60"}`}>
                  /{v.slug}
                </Link>
              ))}
            </span>
